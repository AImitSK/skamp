...
import { findBestCompanyMatches } from './string-similarity-admin';
import { analyzeDatabaseSignals } from './database-analyzer-admin';
 ...
 * ⚠️ WICHTIG: Nutzt DIREKTE Firestore Calls statt companiesEnhancedService (Client SDK!)
 */
async function createNewCompany(
  ...
  try {
    // ✅ Admin SDK: Direkte Firestore Calls (KEIN companiesEnhancedService!)
    const companyData: any = {
      ...
      isGlobal: autoGlobalMode, // ✅ Direkt setzen statt Service
    ...
    const companyRef = await adminDb.collection('companies_enhanced').add(companyData);
