 ...
 *
 * @param variants - Die zu mergenden Varianten
 * @param baseUrl - Base URL für API Calls (Admin SDK läuft auf Server, braucht absolute URL!)
 */
export async function mergeVariantsWithAI(
  ...
  try {
    // ✅ Admin SDK läuft auf Server → braucht absolute URL!
...
function mechanicalMerge(variants: MatchingCandidateVariant[]): MergedContactData {
  ...
  const allMediaTypes = new Set<'print' | 'online' | 'tv' | 'radio' | 'podcast'>();
  variants.forEach(v => {
