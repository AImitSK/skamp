...
export function GlossaryEntryModal({
  ...
  const t = useTranslations('settings.glossaryEntry');
  ...
  const handleSave = async () => {
    ...
    if (!germanTerm.trim()) {
      setError(t('errors.termRequired'));
      return;
    }
    ...
    <Dialog open={isOpen} onClose={onClose} size="lg">
      ...
      <DialogBody>
        <div className="space-y-5">
          ...
          {/* Kunde */}
          <Field>
            <Label>{t('fields.customer.label')}</Label>
            <Select
              value={customerId}
              onChange={(e) => setCustomerId(e.target.value)}
              disabled={saving || isEditing}
            >
              <option value="">{t('fields.customer.placeholder')}</option>
          ...
          {/* Deutscher Begriff */}
          <Field>
            <Label>{t('fields.germanTerm.label')}</Label>
            <Input
              value={germanTerm}
              onChange={(e) => setGermanTerm(e.target.value)}
              placeholder={t('fields.germanTerm.placeholder')}
              disabled={saving}
            />
          </Field>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-base/6 text-zinc-950 font-medium sm:text-sm/6 dark:text-white">
                {t('fields.translations.label')}
              </span>
              <Badge color="purple" className="text-xs">
                <SparklesIcon className="h-3 w-3 mr-1" />
                {t('fields.translations.badge')}
              </Badge>
            </div>
              ...
              {Object.entries(translations).map(([lang, value]) => (
                <div key={lang} className="flex items-center gap-2">
                  ...
                  <Input
                    ...
                    placeholder={t('fields.translations.placeholder', { language: LANGUAGE_NAMES[lang as LanguageCode] || lang })}
                    disabled={saving}
                    className="flex-1"
                  />
                  <button
              ...
              {showLanguageSelector ? (
                <div className="flex items-center gap-2">
                  <Select
                    ...
                    className="flex-1"
                  >
                    <option value="">{t('addLanguage.placeholder')}</option>
          ...
          {/* Kontext */}
          <Field>
            <Label>{t('fields.context.label')}</Label>
            <Textarea
              value={context}
      ...
      <DialogActions>
        ...
        >
          {saving ? t('actions.saving') : isEditing ? t('actions.save') : t('actions.create')}
        </Button>
      </DialogActions>
    </Dialog>
  );
